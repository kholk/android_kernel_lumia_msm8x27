...
void hdmi_msm_phy_status_poll(void)
{
	...
	if (lock_det) {
		pr_info("HDMI Phy PLL Lock Detect Bit is set\n");
	} else {
		pr_info("HDMI Phy Lock Detect Bit is not set,"
	...
	if (phy_ready) {
		pr_info("HDMI Phy Status bit is set and ready\n");
	} else {
		pr_info("HDMI Phy Status bit is not set,"
